...
$nots = $db->query("SELECT * FROM nots WHERE not_user='{$user_email}'");
$notArray = [];
if ( $nots->rowCount() ){
    ...
    foreach( $nots as $not ){
        ...
        $not_date = $not["not_date"];
        $notDetail=array(
            "notSubject" => $not_subject,
            "notDate" => $not_date
        );
        array_push($notArray,$notDetail);
    }
    $not_number = $nots->rowCount();

}else{
    
}
$listNots = json_encode($notArray);
...
<head>
    ...
    <title>Notlarım - Codingo</title>
    
    <!--Link Tag-->
    <link rel="stylesheet" href="../css/reset.css" type="text/css">
    <link rel="stylesheet" href="../css/header.css" type="text/css">
    <link rel="stylesheet" href="../css/nots.css" type="text/css">
</head>
<body>
<div class="header">
...
<div class="subjectArea">
    ...
    <script>
        ...
        const notArray = <?php echo $listNots?>;
        let mathNote = [];
        let pyNote = [];
        let notNo,notInfo;
        if(notArray.length === 0){
            document.querySelector(".myNots").textContent = "Hiç notunuz bulunmamaktadır."
        }else{

        }
        //Not listesini matematik ve python için bölmek
        notArray.forEach(element => {
            if(element.notDate[0] == "M"){
                notNo = parseInt(element.notDate.split(" ")[1].split(".")[0])
                notInfo = {
                    "notSubject" : element["notSubject"],
                    "notNo" : notNo
                }
                mathNote.push(notInfo)
            }else{
                notNo = parseInt(element.notDate.split(" ")[1].split(".")[0])
                notInfo = {
                    "notSubject" : element["notSubject"],
        ...
        //Matematik için div oluşturmak
        mathNoMaxList = []
        mathNote.forEach(element => {
            mathNoMaxList.push(element.notNo)
        });
        let mathNoMax = Math.max(...mathNoMaxList)
        for (let index = 0; index < mathNoMax; index++) {
            let mathNot = $(`<div id='myNote' data-site-key='Math ${index+1}' class='closedStatus Math${index+1}'></div>`)
            $(".myNots").append(mathNot)
        }
        //Python için div oluşturmak
        PyNoMaxList = []
        pyNote.forEach(element => {
            PyNoMaxList.push(element.notNo)
        });
        let PyNoMax = Math.max(...PyNoMaxList)
        for (let index = 0; index < PyNoMax; index++) {
            let PyNot = $(`<div id='myNote' data-site-key='Py ${index+1}' class='closedStatus Py${index+1}'></div>`)
            $(".myNots").append(PyNot)
        }
        let  myNots= document.querySelectorAll(".myNots #myNote")
        myNots.forEach(element => {
            let siteKey = element.getAttribute("data-site-key")
            if(siteKey[0] == "M"){
                let index = parseInt(siteKey.split(" ")[1])
                let value = `Matematik ${index}. Ders`
                let dataValue = $(`<div id="lessonCapiFeminizm"></div>`).text(value)
                $(element).append(dataValue)
            }else{
                let index = parseInt(siteKey.split(" ")[1])
                let value = `Python ${index}. Ders`
                let dataValue = $(`<div id="lessonCapiFeminizm"></div>`).text(value)
                $(element).append(dataValue)
            }
        });
        //Matematiği ekrana yazdırma
        mathNote.forEach(key => {
            
            let keyNo = key["notNo"]
            let section = document.querySelector(`.Math${keyNo}`)
            let noteMathSection = $(`<div class="myNoteSection"></div>`)
            let star = $(`<img src='../img/Star.svg'>`)
            let subject = $(`<div id='myNoteSectionPart${keyNo}' class='noteSubject'></div>`).text(key["notSubject"])
            $(noteMathSection).append(star,subject)
            $(section).append(noteMathSection)
        });
        //Python ekrana yazdırma
        pyNote.forEach(key => {
            let keyNo = key["notNo"]
            let section = document.querySelector(`.Py${keyNo}`)
            let notePySection = $(`<div class="myNoteSection"></div>`);
            let star = $(`<img src='../img/Star.svg'>`);
            let subject = $(`<div id='myNoteSectionPart${keyNo}' class='noteSubject'></div>`).text(key["notSubject"]);
            $(notePySection).append(star,subject)
            $(section).append(notePySection)
        });
    </script>
    <script>
        // Header Bölümü
        ...
        $(".signOutForm").on("click",(e)=>{
            var cookies = document.cookie.split(";")
            const DeleteEmailValue = cookies[0].split("=")[1];
            const DeletePassValue = cookies[1].split("=")[1];
            document.cookie = `email = ${DeleteEmailValue}; expires = Mon, 1 Jan 2000 00:00:00 GMT`
            document.cookie = `password = ${DeletePassValue}; expires = Mon, 1 Jan 2000 00:00:00 GMT`
        })
    </script>
    
    <script type="module" src="../js/headerTheme.js"></script>
    <script type="module">
        ...
        let theme = Decryption(localStorage.getItem("theme"))
        if(theme=="darkTheme"){
            $("#closeThis").attr("fill","white")
        }
